...
namespace AmsApi.Controllers
{
    ...
    public class RequestStatusesController : ControllerBase
    {
        ...
        [HttpPut("SetCompletedStatus")]
        public async Task<ActionResult<RequestDataDto>> SetCompletedStatus(int id, [FromBody] RequestDataDto requestDetails)
        {
            var response = _mapper.Map<Request>(requestDetails);
            var requestNew = await _requestStatusService.SetCompletedStatus(id, response);
            var requestStatus = _mapper.Map<RequestDetailsDto>(requestNew);
            return Ok(requestStatus);
        }
